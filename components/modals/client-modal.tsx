...
type Client = { id?: string; name: string; nif: string; address?: string | null; phone?: string | null; email?: string | null };

export default function ClientModal({ onClose, onSaved, initial }: {
  onClose: () => void;
  onSaved: (c: any, mode: 'create' | 'update') => void;
  initial?: Client | null;
}) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(isEdit ? 'Cliente actualizado' : 'Cliente criado');
      onSaved(j.client, isEdit ? 'update' : 'create');
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-card rounded-md shadow-lg w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-3 border-b">
          <h3 className="font-semibold">{isEdit ? 'Editar cliente' : 'Novo cliente'}</h3>
